...
def get_database_url():
    """Return Supabase/PostgreSQL URL from Streamlit secrets or environment."""
    try:
...
def ensure_data_files_csv_only():
    """CSV setup only. Does not call Supabase. Used for fallback and DB seeding."""
            ...
            for name in REQUIRED_FILES:
                ...
                if int(count or 0) == 0:
                    ...
                    if not seed_df.empty:
                        seed_df = normalize_required_columns(name, seed_df).astype(str)
...
def write_table_db(name, df):
    ...
    try:
        ...
        with engine.begin() as conn:
            create_db_table_if_needed(conn, name)
            # CSV-like save behavior: replace rows, but do not drop/recreate schema.
...
def get_csv_row_counts():
    ...
    for name in REQUIRED_FILES:
        try:
            ...
            count = "Error"
        rows.append({"Table": name, "CSV Rows": count})
...
def get_schema_alignment_report():
    engine = get_db_engine()
    rows = []
    if engine is None:
        return pd.DataFrame([{"Table": name, "Schema Status": "No DATABASE_URL"} for name in REQUIRED_FILES])
    try:
        ensure_database_tables()
        with engine.begin() as conn:
            for name, required_cols in REQUIRED_FILES.items():
                table = db_table_name(name)
                existing_cols = [r[0] for r in conn.execute(text("""
                    SELECT column_name FROM information_schema.columns
                    WHERE table_name = :table_name
                ...
                extra = [c for c in existing_cols if c not in required_cols]
                rows.append({
                    "Table": name,
                    "Schema Status": "OK" if not missing else "Missing Columns",
...
def export_supabase_to_csv():
    ...
    for name in REQUIRED_FILES:
        ...
        df = normalize_required_columns(name, df)
        write_table_csv(name, df)
    return "Supabase exported to local CSV files successfully."
...
def db_connection_status_text():
    ...
    engine = get_db_engine()
    if engine is None:
        return "DATABASE_URL configured but engine not created"
    try:
        with engine.connect() as conn:
            conn.execute(text("SELECT 1"))
        return "Supabase PostgreSQL connected"
    except Exception as e:
...
def database_health_panel():
    ...
    st.caption("Use this section to validate Supabase parity with the CSV baseline before client use.")
    ...
    csv_counts = get_csv_row_counts()
    db_counts = get_db_row_counts()
    counts = csv_counts.merge(db_counts, on="Table", how="outer")
    ...
    for table, expected_rows in expected.items():
        ...
        db_value = row["DB Rows"].iloc[0] if not row.empty and "DB Rows" in row.columns else ""
        csv_value = row["CSV Rows"].iloc[0] if not row.empty and "CSV Rows" in row.columns else ""
        validation_rows.append({
            "Table": table,
            "Expected Minimum": expected_rows,
            "CSV Rows": csv_value,
            "DB Rows": db_value,
            "Status": "OK" if str(db_value).isdigit() and int(db_value) >= expected_rows else ("CSV OK / DB not active" if str(csv_value).isdigit() and int(csv_value) >= expected_rows and not str(db_value).isdigit() else "Check"),
        })
    ...
    st.info("Acceptance target: employees=7, leave_entries=62, advance_cases=6, advance_schedule=6, users=1 in Supabase. Then generate payroll and verify Salary Summary.")
...
def system_health_page():
    ...
    st.caption(f"Database Mode: {db_connection_status_text()}")
