...
3. Recommended Supabase session pooler URL format for Streamlit Cloud:
   DATABASE_URL = "postgresql+psycopg2://postgres.YOUR_PROJECT_REF:YOUR_DATABASE_PASSWORD@aws-1-ap-south-1.pooler.supabase.com:5432/postgres?sslmode=require"
"""
...
def normalize_database_url(raw_url: str) -> str:
    ...
    """
    url = (raw_url or "").replace("\x00", "").replace("%00", "").replace("%2500", "").strip().strip('"').strip("'")
...
REQUIRED_TABLES = ["employees", "attendance", "payroll_runs", "sms_users"]


def get_existing_tables() -> set[str]:
    """Return the SMS tables that already exist in the public schema."""
    engine = get_engine()
    if engine is None:
        return set()

    table_list = "'employees', 'attendance', 'payroll_runs', 'sms_users'"
    with engine.connect() as conn:
        result = conn.execute(
            text(
                f"""
...
def initialize_sms_tables(force: bool = False) -> None:
    """
    Create/repair SMS tables safely.

    Why this is designed this way:
    - Streamlit reruns the script frequently.
    - Repeated CREATE TABLE calls from multiple sessions can hit Supabase/Postgres
      statement timeouts or catalog locks.
    - Therefore we check existing tables first, run setup only if needed, and use
      an advisory lock so only one app session runs DDL at a time.
    """
    engine = get_engine()
    if engine is None:
        raise RuntimeError("DATABASE_URL missing. Cannot initialize Supabase tables.")

    if not force and not get_missing_tables():
        return
    ...
    ddl_statements = [
        ...
        "ALTER TABLE public.employees ADD COLUMN IF NOT EXISTS employee_name TEXT;",
        "ALTER TABLE public.employees ADD COLUMN IF NOT EXISTS level TEXT DEFAULT 'L1';",
        "ALTER TABLE public.employees ADD COLUMN IF NOT EXISTS monthly_salary NUMERIC(12,2) DEFAULT 0;",
        "ALTER TABLE public.employees ADD COLUMN IF NOT EXISTS paid_leave_quota INTEGER DEFAULT 2;",
        ...
        "CREATE UNIQUE INDEX IF NOT EXISTS employees_emp_id_unique_idx ON public.employees (emp_id);",
        """
        ...
        "ALTER TABLE public.attendance ADD COLUMN IF NOT EXISTS emp_id TEXT;",
        ...
        "ALTER TABLE public.attendance ADD COLUMN IF NOT EXISTS supervisor TEXT;",
        ...
        "CREATE UNIQUE INDEX IF NOT EXISTS attendance_date_emp_id_unique_idx ON public.attendance (attendance_date, emp_id);",
        """
        ...
        """,
        "ALTER TABLE public.sms_users ADD COLUMN IF NOT EXISTS email TEXT;",
        "ALTER TABLE public.sms_users ADD COLUMN IF NOT EXISTS name TEXT;",
        "ALTER TABLE public.sms_users ADD COLUMN IF NOT EXISTS role TEXT DEFAULT 'admin';",
        "ALTER TABLE public.sms_users ADD COLUMN IF NOT EXISTS password_hash TEXT;",
        "ALTER TABLE public.sms_users ADD COLUMN IF NOT EXISTS active BOOLEAN DEFAULT TRUE;",
        "ALTER TABLE public.sms_users ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ DEFAULT NOW();",
        "ALTER TABLE public.sms_users ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT NOW();",
        "CREATE UNIQUE INDEX IF NOT EXISTS sms_users_email_unique_idx ON public.sms_users (email);",
    ]

    with engine.begin() as conn:
        # Works with Supavisor session mode / port 5432. It only applies to this transaction.
        conn.execute(text("SET LOCAL statement_timeout = '60s';"))
        # Prevent multiple Streamlit reruns/sessions from performing DDL simultaneously.
        conn.execute(text("SELECT pg_advisory_xact_lock(hashtext('sms_table_setup_lock'));"))

        for ddl in ddl_statements:
            conn.execute(text(ddl))

    missing_after_setup = get_missing_tables()
    if missing_after_setup:
        raise RuntimeError(f"Supabase table setup incomplete. Missing tables: {missing_after_setup}")


@st.cache_resource(show_spinner=False, ttl=3600)
def ensure_sms_tables_cached(database_url_fingerprint: str) -> tuple[bool, str]:
    """Cache table setup so Streamlit reruns do not repeatedly execute CREATE TABLE."""
    try:
        initialize_sms_tables(force=False)
        return True, "SMS tables are ready."
    except Exception as exc:
        return False, str(exc)
...
if connected:
    setup_ok, setup_message = ensure_sms_tables_cached(str(hash(normalize_database_url(get_secret_value("DATABASE_URL")))))
    if setup_ok:
        st.success("✅ Supabase database connected and SMS tables are ready.")
    else:
        connected = False
        db_message = (
            "Supabase login worked, but SMS table setup did not complete. "
            "The app is using local CSV fallback for this session.\n\n"
            "Most likely cause: a temporary Supabase/Postgres DDL statement timeout or catalog lock while creating tables.\n\n"
            f"Raw error:\n{setup_message}"
        )
...
with st.sidebar:
    ...
    if connected:
        if st.button("Create / Repair SMS Tables", use_container_width=True):
            try:
                initialize_sms_tables(force=True)
                ensure_sms_tables_cached.clear()
